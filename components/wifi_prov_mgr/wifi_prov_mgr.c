...
static void wifi_prov_print_qr(const char *name, const char *username, const char *pop, const char *transport, char* url)
{
    if (!name || !transport) {
...
#ifdef CONFIG_EXAMPLE_PROV_TRANSPORT_BLE
        wifi_prov_print_qr(service_name, username, pop, PROV_TRANSPORT_BLE, NULL);
#else /* CONFIG_EXAMPLE_PROV_TRANSPORT_SOFTAP */
        wifi_prov_print_qr(service_name, username, pop, PROV_TRANSPORT_SOFTAP, NULL);
#endif /* CONFIG_EXAMPLE_PROV_TRANSPORT_BLE */
    } else {
...
{
    /* Initialize the Wi-Fi provisioning manager */
    /* Initialize NVS partition */
    esp_err_t ret = nvs_flash_init();
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        /* NVS partition was truncated
...
void wifi_bt_net_run(char* url)
{
       ESP_LOGI(TAG, "Starting provisioning");
        ...
        /* Print QR code for provisioning */
#ifdef CONFIG_EXAMPLE_PROV_TRANSPORT_BLE
        wifi_prov_print_qr(service_name, username, pop, PROV_TRANSPORT_BLE, url);
#else /* CONFIG_EXAMPLE_PROV_TRANSPORT_SOFTAP */
        wifi_prov_print_qr(service_name, username, pop, PROV_TRANSPORT_SOFTAP, url);
#endif /* CONFIG_EXAMPLE_PROV_TRANSPORT_BLE */
}
